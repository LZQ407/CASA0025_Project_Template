...
Built on Google Earth Engine, this application provides an interactive platform for conservation practitioners to explore crane hotspots. Users select a country and province, set a density threshold, and view hotspots displayed in two layers: fully inside reserves (green) and intersecting/outside reserves (orange). This allows users to visualize not only general hotspot distribution but also identify areas where reserve boundaries could be extended. Clicking any hotspot opens a panel showing its peak crane season and averaged NDVI, temperature, pollution (NO₂), and water-area fraction. Environmental layers update dynamically to provide geographic context.

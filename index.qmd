...
Migratory species like the White-naped Crane (Grus vipio) depend on a chain of seasonal habitats across East Asia, yet many critical stopover and staging sites lie wholly outside established nature reserves and remain vulnerable to agricultural expansion, urban development and climate variability (Wilcove & Wikelski 2008). Because these areas are only occupied for brief seasonal windows and span multiple jurisdictions, conservationists struggle to assemble the compelling, spatiotemporally precise evidence needed to justify their formal protection (Yanco et al. 2024; Runge et al. 2014).
...
Crane hotspots are extracted by summing GPS-tagged point counts and convolving them with a Gaussian kernel (10 km radius, σ = 10 km) to produce a smooth density surface (Yanco et al., 2024). Pixels above a user-defined percentile threshold are then masked, passed through a 50 m focal-maximum filter, and vectorized into polygons representing hotspots. Each hotspot is enriched by sampling seasonal mean NDVI, 2 m air temperature, tropospheric NO₂, and water-area fraction. Overlap with protected-area boundaries is quantified to classify each hotspot’s reserve coverage. This integrated workflow links crane movement patterns with habitat quality, supporting evidence-based off-reserve conservation planning.

### Interface

Built on Google Earth Engine, this application provides an interactive platform for conservation practitioners to explore crane hotspots. First, users select a country and then a province to define their study region. A density slider lets them set a threshold between the 99th and 100th percentile, highlighting only the highest-density crane areas. Then, clicking any hotspot opens a panel displaying its peak crane season and that season’s averaged vegetation (NDVI), temperature, pollution (NO₂), and water-area fraction. Underlying environmental layers refresh automatically to provide geographic context. These intuitive steps transform complex spatiotemporal data into clear, shareable evidence for off-reserve habitat protection.
...
Yanco, S. W., Rutz, C., Abrahms, B., Cooper, N. W., Marra, P. P., Mueller, T., *et al.* (2024). Tracking individual animals can reveal the mechanisms of species loss. *Trends in Ecology & Evolution*. Advance online publication. https://doi.org/xxxx  
